...
import numpy as np
import sys
sys.path.append('/home/tangshuo/AI_summer/LAL_Parser/src_joint')
import torch
...
class ChartParser(nn.Module):
    ...
    def from_spec(cls, spec, model):
        ...
        if 'bert_do_lower_case' not in hparams:
            hparams['bert_do_lower_case'] = True
        ...
        if 'xlnet_model' not in hparams:
            hparams['xlnet_model'] = "xlnet-large-uncased"
        if 'xlnet_do_lower_case' not in hparams:
            hparams['xlnet_do_lower_case'] = True
